...
export default function PhimHoatHinh({ params })
{
  ...
  useEffect(() =>
  {
    ...
    {
      const response = await fetch(`https://phimapi.com/v1/api/the-loai/${params.slug}?page=${page}`)
      if (!response.ok)
      {
  ...
  return <main className={styles.phimle}>
    <h1 className={styles.heading}> Phim {title} </h1>
